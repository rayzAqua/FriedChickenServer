...
class Ingredient {
  ...
  static async getIngredientList(
    ...
    try {
      ...
      const ingredientList = await query(sp, [
        ingredientId,
        k3y,
        wareHouseId,
        page_limit,
  ...
  static async create(name, unit, image, createdUser) {
    try {
      const sp = "CALL sp_add_ingredient(?, ?, ?, ?);";
      const ingredient = await query(sp, [name, unit, image, createdUser]);
  ...
  static async update(ingredientId, name, unit, image, updatedUser) {
    try {
      const sp = "CALL sp_update_ingredient(?,?, ?, ?, ?);";
      const ingredient = await query(sp, [
        ingredientId,
        name,
        unit,
        image,
