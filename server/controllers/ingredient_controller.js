...
export const getIngredientList = async (req, res, next) => {
  ...
  try {
    ...
    const getIngredient = await Ingredient.getIngredientList(
      ingredientId,
      k3y,
      wareHouseId,
      page_limit,
    ...
    const filterIngredientArray = Array.isArray(getIngredient[0])
      ? getIngredient[0]
      : [getIngredient[0]];

    // Lọc và lấy ra tổng số lượng các mẫu dữ liệu có trong bảng ingredient.
    const totalIngredients = Array.isArray(getIngredient[1])
      ? getIngredient[1]
      : [getIngredient[1]];
    ...
    const total_page = Math.ceil(
      totalIngredients[0].total_ingredients / page_limit
    );
