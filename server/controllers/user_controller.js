...
export const updateUser = async (req, res, next) => {
  ...
  try {
    ...
    const updateUser = await User.updateUser(
      ...
      image,
      status,
      roleId !== null ? JSON.stringify(roleId) : null,
      updatedUser,
      updatedTime
    );
    const filterUpdateUser = Array.isArray(updateUser[0])
      ? updateUser[0]
      : [updateUser[0]];
    const filterUpdateRole = Array.isArray(updateUser[1])
      ? updateUser[1]
      : [updateUser[1]];
...
export const getUserList = async (req, res, next) => {
  ...
  try {
    ...
    const filterUserArray = Array.isArray(getUsers[0])
      ? getUsers[0]
      : [getUsers[0]];
    ...
    if (filterUserArray.length > 0) {
      const newUserArray = await Promise.all(
        filterUserArray.map(async (user) => {
          const {
            createdUser,
            createdTime,
            updatedUser,
