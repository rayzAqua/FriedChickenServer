...
async function getDetailOrder(res, result, isShow, page, totalPage) {
    ...
    try {
        const [detailResponses, customerResponses, promotionResponses, userResponses, userCancleResponses] =
            await Promise.all([
        ...
        res.send(message(true, 'Lấy dữ liệu thành công!', result, isShow, +page, totalPage))
    } catch (error) {
        console.log(error)
        ...
        try {
            ...
            if (promoteId || null) {
                ...
                if (customer[0]['point'] < promote[0]['available']) {
                    return res.send(message(false, 'Khách hàng không đủ điểm để áp dụng khuyến mãi!', ''))
                }
            }
            ...
            if (paymentMethod.length == 0) {
                return res.send(message(false, 'Phương thức thanh toán không hợp lệ!', ''))
            }
            ...
            for (var i = 0; i < details.length; i++) {
                ...
                ingredientFood.map((ingredient, index) => {
                    ...
                    if (ingredient['available'] == 0) {
                        return res.send(message(false, 'Nguyên liệu hiện không khả dụng!', ''))
                    }
                })
            ...
            //create order
            let order = await Order.create(totalMoney, userId, customerId, promoteId, paymentMethodId)
        ...
        try {
            ...
            if (status == 'Cancelled') {
                ...
                if (!promoteId) {
                    ...
                    let point = Number(customer[0]['point']) - Math.round(Number(order[0]['totalMoney']) / 30000)
                    if (promotion[0]) point += Number(promotion[0]['requirePoint'])
